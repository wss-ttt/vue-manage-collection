<template>
  <div>
    <el-table :data="formdata" style="width: 100%">
          ...
          <div :style="`padding-left:${20*(scope.row.num-1)}px`">
            <span style="display:inline-block;width:14px;height:14px;">
              <i
                ...
                @click="rowclick( scope.row, scope.column, scope.$index )"
                v-if="scope.row.children && !scope.row.flag"
              ></i>
              <i
                style="cursor: pointer;"
                class="el-icon-caret-bottom"
                @click="cellclick( scope.row, scope.column, scope.$index )"
                v-if="scope.row.children && scope.row.flag"
              ></i>
            </span>
            {{scope.row.label}}
      ...
      <el-table-column prop="label" label="部门类型" width="100">
        <template slot-scope="scope">
